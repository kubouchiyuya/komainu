...
[English](README.md) · [Docs](skills/komainu/docs/index.ja.md) · [脅威モデル](skills/komainu/references/threat-model.md)
...
匿名 clone し、第三者コードが*エージェント*を攻める手口を全ファイルから読み、
...
- **攻撃者の武器化のしかたでコードを読む** — 5カテゴリ: 隠し AI 命令（注入）・
  自動実行・情報流出・偽装マルウェア・そして AI 時代の必殺技 **「ガードを書き換えて
  守り自体を切る」**。
...
sh skills/komainu/tests/smoke.sh               # オフライン10アサーション
```
...
10フェーズ・フェーズ2〜5は**一切コードを走らせない**。遮断→最小匿名clone（SHA固定・
