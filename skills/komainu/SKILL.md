...
description: |
  ...
  プラグインを取り込む前に「匿名clone → 完全網羅スキャン → 無害化 → 判定ゲート →
...
guardrails:
  must:
    - "取り込みは必ず komainu import 経由（匿名 HTTPS clone・fork/star/push しない）"
  ...
  requires_approval:
    ...
    - "DANGER 判定の override"
    - "AI が元repoを star する（既定は匿名維持・--star opt-in）"
...
| P2 | 最小・匿名 clone（`--no-checkout`→attributes/modules 先検査→filter/hook 無効化・submodule非再帰・junk除外・.git破棄・SHAピン） | 不実行 |
| P3 | 完全スキャン（注入 / 自動実行 / exfil / malware / ガードレール改竄の5カテゴリ） | 不実行 |
...
```bash
sh .claude/skills/komainu/tests/smoke.sh   # 決定論・オフライン・10アサーション
```

## 関連

- `references/threat-model.md` — 4+1 カテゴリの完全な脅威分類と残余リスク
