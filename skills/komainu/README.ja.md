...
- 静的スキャンは既知パターン中心。難読化・未知は漏れうる。本体は
  **取り込みコードを一切実行しない** ＋ **clone 時に自動実行ベクターを無効化**。
  走らなければ実害は出ない。
...
- [docs/how-it-works.md](docs/how-it-works.md) — 10フェーズ・5カテゴリ・強制層
- [docs/faq.md](docs/faq.md) — star≠install / SAFE≠実行安全 / bypass / 全AI・全OS
- [references/threat-model.md](references/threat-model.md) — 脅威分類の完全版
